...
class AIExpertAgent:
    ...
    def __init__(self, backend: LLMBackend):
        ...
        self.system_instruction_vi = (
            ...
            "QUY TẮC PHẢI TUÂN THỦ VỀ ĐỊNH DẠNG VĂN BẢN (MÀU CHỮ/KÝ TỰ): Tuyệt đối KHÔNG được sử dụng ký hiệu/định dạng công thức toán học kiểu LaTeX trong câu trả lời (ví dụ: KHÔNG dùng các cặp dấu đô la $...$, các hàm như \\text{...}, \\circ, hoặc các ký hiệu mũ/dưới phức tạp kiểu LaTeX). Hãy dùng chữ thường, số mũ thông thường dễ đọc cho con người (ví dụ: viết 'Mg2+' thay vì '$\\text{Mg}^{2+}$', viết 'Tm' thay vì '$\\text{Tm}$', viết '95°C' thay vì '$95^\\circ\\text{C}$', viết 'Ta - 5°C' thay vì '$T_a - 5^\\circ\\text{C}$').\n"
        ...
        self.system_instruction_en = (
            ...
            "MANDATORY PLAIN-TEXT FORMATTING RULE: Absolutely DO NOT use LaTeX math equations or symbols in your responses (e.g., do not use '$...$', '\\text{...}', '\\circ', or complex LaTeX sub/superscripts). Always use plain, human-readable text for chemical and biological terms (e.g., write 'Mg2+' instead of '$\\text{Mg}^{2+}$', write 'Tm' instead of '$\\text{Tm}$', write '95°C' instead of '$95^\\circ\\text{C}$', and 'Ta - 5°C' instead of '$T_a - 5^\\circ\\text{C}$').\n"
